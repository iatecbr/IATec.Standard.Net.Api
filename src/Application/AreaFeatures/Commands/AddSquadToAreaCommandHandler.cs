...
public class AddSquadToAreaCommandHandler(IAreaRepository areaRepository, IUnitOfWork uow, IValidator<Squad> squadValidator) 
    : IRequestHandler<AddSquadToAreaCommand, Response<bool>>
{
    ...
    {
        var area = await areaRepository.GetAreaById(command.AreaId);
        if (area is null)
